...
public class Levels {
	...
	private void loadPixelMap(String filename, World world, Vector2 ddSpawnPos, Vector2 hhSpawnPos) throws IOException {
		...
		wallsBuffer= new byte[(width+2) * (height+2)]; // buffer for boundary of entire array of walls
		for(int i = 0; i< (width+2) * (height+2); i++) wallsBuffer[i] = -1; // flush buffer map
		...
		for(int j = 1;j< height+1;j++){
			for(int i = 1;i<width+1;i++){
				...
				for(byte itype = 0; itype < 10; itype ++){ // loop over all possible types of terrain
					if(wallsBuffer[i + j*(width+2)] == itype){
						if(wallsBuffer[i+1 + j*(width+2)] != itype || wallsBuffer[i-1 + j*(width+2)] != itype || wallsBuffer[i + (j+1)*(width+2)] != itype || wallsBuffer[i + (j-1)*(width+2)] != itype){
